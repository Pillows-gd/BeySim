...
public:
    ...
    // !! MY PUBLIC CODE STARTS FROM HERE !!
    glm::vec3 getBoundingBox()
    {
        glm::vec3 minCoords = glm::vec3(0.0f);
        glm::vec3 maxCoords = glm::vec3(0.0f);
        for(GLuint i = 0; i < this->meshes.size(); i++)
        {
            for (GLuint j = 0; j < 3; j++)
            {
                for (GLuint v = 0; v < this->meshes[i].vertices.size(); v++)
                {
                    minCoords[j] = this->meshes[i].vertices[v].Position[j] < minCoords[j] ? this->meshes[i].vertices[v].Position[j] : minCoords[j];
                    maxCoords[j] = this->meshes[i].vertices[v].Position[j] > maxCoords[j] ? this->meshes[i].vertices[v].Position[j] : maxCoords[j];
                }
            }
        }
        cout << "size of the loading model's bounding box (if numbers are weird the loading must have failed):\n";
        cout << "x size: " << maxCoords[0]-minCoords[0] << " y size: " << maxCoords[1]-minCoords[1] << " z size: " << maxCoords[2]-minCoords[2] << "\n";
        return glm::vec3(maxCoords[0]-minCoords[0],maxCoords[1]-minCoords[1],maxCoords[2]-minCoords[2]);
    }

    GLuint getBoundingSphereRay()
    {
    ...
    Mesh processMesh(aiMesh* mesh)
    {
        ...
        for(GLuint i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            if(mesh->mTextureCoords[0])
            {
                ...
                cout << (mesh->mName).C_Str() << "WARNING::ASSIMP:: MODEL WITHOUT UV COORDINATES -> TANGENT AND BITANGENT ARE = 0" << endl;
            }
