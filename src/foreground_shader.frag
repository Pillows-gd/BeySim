...
vec4 Lambert() // this name is the one which is detected by the SetupShaders() function in the main application, and the one used to swap subroutines
{
    ...
    return vec4(Kd * lambertian * diffuseColor, 1.0);
}
//////////////////////////////////////////
...
subroutine(ill_model)
vec4 Phong() // this name is the one which is detected by the SetupShaders() function in the main application, and the one used to swap subroutines
{
...
vec4 GGX() // this name is the one which is detected by the SetupShaders() function in the main application, and the one used to swap subroutines
{
    ...
    for (int i=0; i < lightDir.length(); i++)
    {
        ...
        vec3 lambert = (Kd*diffuseColor*lightColor[i])/PI;
        ...
        if(NdotL > 0.0)
        {
            ...
            specular = lightColor[i]*(F * G2 * D) / (4.0 * NdotV * NdotL);
        }
        result += (lambert + specular)*NdotL;
    }
...
subroutine(ill_model)
vec4 GGX_TX() // this name is the one which is detected by the SetupShaders() function in the main application, and the one used to swap subroutines
{
    vec3 result;
    // normalization of the per-fragment normal
    vec3 N = normalize(vNormal);
...
{    
    ...
    for (int i=0; i < lightDir.length(); i++)
    {
        ...
        vec4 sampledColor = texture(tCube, R);
        vec3 lambert = (Kd*sampledColor.rgb)/PI;
...
vec4 Particles()
{
    return vec4(particleColor.xyz, 1.0);
}

vec4 computeFog(vec4 baseColor)
{
    // interp(x,y,a) = x*a + y*(1-a), 0 < a < 1
    vec3 V = worldPosition.xyz - cameraPosition;
    float a = length(V) * 0.05 > 1.0 ? 1.0 : length(V) * 0.05;
    return vec4(
        a*0.5 + (1.0-a)*baseColor.x,
        a*0.5 + (1.0-a)*baseColor.y,
        a*0.8 + (1.0-a)*baseColor.z,
        1.0
    );
}
...
void main(void)
{
    ...
    if (fogFlag)
        colorFrag = computeFog(Illumination_Model());
    else
